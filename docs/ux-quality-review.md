...
> **Status**: every Critical and High finding below has been fixed and re-measured
> against the running application. Those rows are marked **Fixed** with the
> measurement that now holds. Medium and Low findings are open. See
> [What changed](#what-changed) at the end for the diff-level summary.
...
The one deliberate trade-off: the map now opens showing fewer nodes, because it
will no longer zoom out past the point where its own text is readable. That is
what F-002's cue is for. Medium and Low findings — heading levels, 34×34 icon
buttons, the `title`-only vocabulary, the prototype's unreachable `3 of 6`, line
length on record pages — remain open.
